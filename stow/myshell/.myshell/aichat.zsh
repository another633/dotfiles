...
# aichat：将当前 BUFFER 作为输入，打印在下一行并输出结果，

chat_prompt="你是一个温和、聪明的中文助手。
与你进行日常对话时，请保持自然、友善的语气；
当遇到专业或技术性问题时，请用简洁、直接的方式回答，不要过度展开。
如有必要，可给出一句话的解释或示例，但不要写成长篇说明。
除非我明确要求，否则保持回答简洁。
始终使用简体中文回答。"
...
_aichat_chat() {
  ...
  # 同步执行 aichat，并把输出直接打印到终端
  aichat --prompt "$chat_prompt" "$orig"
...
# 注册为 ZLE widget 并绑定 Alt+c（ESC c）
zle -N _aichat_chat
bindkey '\ec' _aichat_chat
